...
namespace IKEssentials
{
    public class FullArmTargetHandler
    {
        ...
        public float GetHandWeight() => _handSolver.GetCurrentWeight();
        public Vector3 GetHandPosition() => _handSolver.SolverPosition;

        public void ForceWeight(float target, bool handRotation = true, bool bend = false)
        {
            _handSolver.SetWeight(target);
            _fingersSolver.FingersWeight = target;
            if(handRotation)
                _handSolver.SetRotationWeight(target);
            if (bend)
                _bend.weight = target;
        }
        ...
        public void HandleHandTarget(UHandTargetBase targetData, float smoothSpeed)
        {
            ...
            void DoTransformsLerp()
            {
                ...
                if (_currentBendTarget == null) return;
